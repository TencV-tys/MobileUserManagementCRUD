...
`)
  ...
  async addUser(name: string, email: string, age: number): Promise<number> {
    try {
      const res = this.db.runAsync(`INSERT INTO users (name,email,age) VALUES (?,?,?)`, [name, email, age])
  ...
  async getAllUser(): Promise<User[]> {
    try {
      const users = this.db.getAllAsync('SELECT id,name,email,age,created_at FROM users ORDER BY id DESC')
  ...
  async getById(id: number): Promise<User | null> {
    try {
      const user = this.db.getFirstAsync('SELECT * FROM users WHERE id = ?', [id])
  ...
  async updateUser(id: number, name: string, email: string, age: number): Promise<number> {
    try {
      const result = this.db.runAsync('UPDATE users SET name = ?, email = ?, age = ? WHERE id = ?', [
        name, email, age, id
      ])
  ...
  async deleteUser(id: number): Promise<number> {
    try {
      const res = this.db.runAsync('DELETE FROM users WHERE id = ?', [id])
